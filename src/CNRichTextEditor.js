...
class CNRichTextEditor extends Component {
  ...
  addImageContent = (url, id, height, width, type = 0) => {
  ...
  renderImage(image, index, id) {
    ...
    const { ImageComponent = Image, onPressImage } = this.props;
    return (
      <View
        key={`image${index}`}
        style={{
          ...
          >
            {_.map(value, (item, index) => {
              ...
              if (item.component === "image") {
                return this.renderImage(item, index, item.id);
              }
            })}
